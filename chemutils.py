...
# from rdkit.Chem.rdchem.Atom import Atom
# from rdkit.Chem.rdchem.ChiralType import ChiralType
# from rdkit.Chem.rdchem.RWMol import RWMol
...
def get_mol(smiles):  # 输入分子序列，返回分子对象
...
def get_smiles(mol):  # 输入分子对象，返回分子序列
    return Chem.MolToSmiles(mol, kekuleSmiles=True)


def decode_stereo(smiles2D):  # 通过分子序列得到所有同位体的信息
    ...
    if len(chiralN) > 0:
        for mol in dec_isomers:
            for idx in chiralN:
                mol.GetAtomWithIdx(idx).SetChiralTag(Chem.rdchem.ChiralType.CHI_UNSPECIFIED)  # 这里与源代码不同, 后面相同了
            smiles3D.append(Chem.MolToSmiles(mol, isomericSmiles=True))

    return smiles3D


def sanitize(mol):  # 它的功能尚不清楚，个人觉得应该是类似规范化之类的
    try:
...
def copy_atom(atom):  # 复制一个原子，与源代码调用的类有所不同，可能存在问题
    new_atom = Chem.Atom(atom.GetSymbol())  # 这里与源代码不同, 现在相同了
    new_atom.SetFormalCharge(atom.GetFormalCharge())
    new_atom.SetAtomMapNum(atom.GetAtomMapNum())
    return new_atom


def copy_edit_mol(mol):  # 通过重新构建分子的来复制，与源代码调用不同
...
def get_clique_mol(mol, atoms):
    smiles = Chem.MolFragmentToSmiles(mol, atoms, kekuleSmiles=True)  # 把模块转化为分子序列
    new_mol = Chem.MolFromSmiles(smiles, sanitize=False)
    new_mol = copy_edit_mol(new_mol)  # RWMol的作用是一个更强大的Mol，它可以直接进行更改，不用创建新的对象
    new_mol = new_mol.GetMol()  # 通过GetMol()的作用是把RWMol转化为Mol
    new_mol = sanitize(new_mol)  # We assume this is not None
    return new_mol


def tree_decomp(mol):  # 重要函数：把分子结构模块化，返回模块的列表以及边的组合
    n_atoms = mol.GetNumAtoms()
    if n_atoms == 1:  # 如果分子中只有一个原子，则该原子为唯一个模块
        return [[0]], []

    cliques = []
    for bond in mol.GetBonds():  # 把不在环上的化学键两头的原子直接当作一个模块
        a1 = bond.GetBeginAtom().GetIdx()
        a2 = bond.GetEndAtom().GetIdx()
        if not bond.IsInRing():
            cliques.append([a1, a2])

    ssr = [list(x) for x in Chem.GetSymmSSSR(mol)]  # 这一步的作用还不清楚，ssr的类型是[[]]，而其它的类型只是[]
    cliques.extend(ssr)

    # 统计每个原子所在模块的分布
    nei_list = [[] for _ in range(n_atoms)]
    for i in range(len(cliques)):
        for atom in cliques[i]:  # 这一步是否直接可行？atom应该是一个原子类的对象，能不能用来作为下标遍历？
            nei_list[atom].append(i)

    # Merge Rings with intersection > 2 atoms
    for i in range(len(cliques)):
        if len(cliques[i]) <= 2:  # 不是环的情况跳过
            continue
        for atom in cliques[i]:
            for j in nei_list[atom]:  # 这里报错很离谱
    ...
    junc_tree = minimum_spanning_tree(clique_graph)  # 这一步有警告，但应该没出错
    row, col = junc_tree.nonzero()
    edges = [(row[i], col[i]) for i in range(len(row))]
    return cliques, edges


def atom_equal(a1, a2):  # 一个原子是否相同，当且仅当元素种类相同，电荷数相同
    return a1.GetSymbol() == a2.GetSymbol() and a1.GetFormalCharge() == a2.GetFormalCharge()


# Bond type not considered because all aromatic (so SINGLE matches DOUBLE)
def ring_bond_equal(b1, b2, reverse=False):  # 检查两条化学键是否相同
...
def enum_assemble(node, neighbors, prev_nodes=None, prev_amap=None):
    ...
    def search(cur_amap, depth):
        ...
        candidate_list = []
        for search_amap in cand_amap:
            search_cand_mol = local_attach(node.mol, neighbors[:depth + 1], prev_nodes, search_amap)
            search_cand_mol = sanitize(search_cand_mol)
            if search_cand_mol is None:
                continue
            search_smiles = get_smiles(search_cand_mol)
            if search_smiles in candidate_smiles:
                continue
            candidate_smiles.add(search_smiles)
            candidate_list.append(search_amap)

        if len(candidate_list) == 0:
            return

        for new_amap in candidate_list:
